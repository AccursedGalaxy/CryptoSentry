...
async def on_guild_join(guild):
    ...
    await guild.system_channel.send(
        "Hello! I'm a bot that can help you with your DCA levels."
    )
    await guild.system_channel.send(
        "To get started, type `!setchannel <channel_id>` to set the channel for signals."
    )
...
async def on_command_error(ctx, error):
    ...
    if isinstance(error, commands.MissingPermissions):
        ...
        await ctx.send(
            f"{ctx.author.mention} This command is on cooldown. Please try again in {error.retry_after:.2f}s."
        )
    elif isinstance(error, commands.CommandInvokeError):
        await ctx.send(
            f"{ctx.author.mention} An error occurred while invoking the command. Please try again later."
        )
    elif isinstance(error, commands.CommandNotFound):
        pass
    else:
        await ctx.send(
            f"{ctx.author.mention} An error occurred while processing the command. Please try again later."
        )
